package pointFunctions;
 ...
 /* Lineal Path	- The likelihood of a line of length L
  will lie completely in the selected component.*/
...
public class Lineal_Path_Function implements PlugInFilter, DialogListener
{
	...
	private DialogParams DoMyDialog()
	{
		...
		GenericDialog gd = new GenericDialog("Lineal Path Distribution");
		GenericDialogAddin gda = new GenericDialogAddin();
		gd.addRadioButtonGroup("Value to process", valChoices, 1, 2, valChoices[0]);
		gd.addChoice("Calculate for ",sliceChoices,sliceChoices[0]);
	...
	public void run(ImageProcessor ip)
	{
		...
		if(dp!=null)
		{
			String title,caption, capStr= "Lineal Path Distribution";
			...
			case "Current Slice":
				...
				hist1= pf.linealPathDistribution(image,imgWidth,imgHeight,imgDepth,
						pixelWidth,pixelHeight,pixelDepth,
						dp.nSamples, dp.valChoice, dp.showProgress);
				sliceImp.close();
				
				title = imp.getShortTitle() + "_LinPath_Slice" + imp.getCurrentSlice() + "_N"+dp.nSamples+"_"+ dp.valChoice +".tif";
			...
			case "All Slices 2D":
				...
				for(int i = 0; i<slices; i++)
				{
					...
					hist1= pf.linealPathDistribution(image,imgWidth,imgHeight,imgDepth,
							pixelWidth,pixelHeight,pixelDepth,
							dp.nSamples, dp.valChoice, false);
				...
				title = imp.getShortTitle() + "_LinPath_" + dp.sliceChoice + "_N" +dp.nSamples + "_" + dp.valChoice +".tif";
				title = WindowManager.makeUniqueName(title);
				rt.show(title);
				plot = new Plot(title,"Length("+unit+")","Probability");
			...
			case "All Slices 3D":
				...
				hist1= pf.linealPathDistribution(volData,imgWidth,imgHeight,imgDepth,
						pixelWidth,pixelHeight,pixelDepth,
						dp.nSamples, dp.valChoice, dp.showProgress);
